...
pub fn pre_checks() -> (String, String, String, String) {
    ...
    let peer_name = read_file_to_string("./.git/.author-cb/git-cb")
                        .expect("Can't read the local peer name from git-cb file!");
    let peer_email = read_file_to_string("./.git/.author-cb/git-cb-email")
                        .expect("Can't read the local email from git-cb-email file!");
    
    // 2. Check if keys(priv-pub) in .author-cb/ exists
    let openssh_priv_key = read_file_to_string("./.git/.author-cb/key")
    ...
    let pub_key = read_file_to_string("./.git/.author-cb/key.pub")
    ...
    let exists_signers = fs::exists("./.git/.author-cb/allowed_signers")
            .expect("Unable to get --- signers");
    assert!(exists_signers);

    // 4. Check if .author-cb/db.csv exists
    let exists_db = fs::exists("./.git/.author-cb/db.csv")
...
pub fn modify_db_csv_file(gid: &str, new_tree: &str, g_name: &str, modified: bool) {
    ...
    if let Ok(lines) = read_lines_csv("./.git/.author-cb/db.csv") {
        ...
        for line in lines.flatten() {
            ...
            if line.contains(n_gid) {
                ...
                // println!("{:?}",line_spl);
                let new_st = if modified {
                    format!("{},{},{},{},{}",line_spl[0],g_name,line_spl[1],new_tree,line_spl[4])
                } else {
                    format!("{},{},,{},{}",line_spl[0],g_name,new_tree,line_spl[4])
                };
                // println!("{}",new_st);
