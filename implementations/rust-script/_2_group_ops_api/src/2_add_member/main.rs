...
use crate::member_add::add_member_func;
mod member_add;
...
fn main() {
    ...
    // println!("{}, {} performing actions",peer_name, peer_email);
    ...
    if args.len() < 3 {
        panic!("Please provide the remote peer name to add to the group.");
    }
    ...
    // 6. Check if remote peer already exists in the group OR new remote peer is added
    let remote_path_str = format!("{}/{}", group_id, remote_name);
    let tup_value = add_member_func(&repo, &mut curr_index, &remote_path_str, group_name, remote_name);

    // 7. Add to the current index
    add_to_current_index(&repo, &mut curr_index, &remote_path_str, &tup_value)
    .expect("Failed to add new member to the current index!");
    ...
    let commit_message = format!("GROUP_OPS({},add)",format!("{} added to the group {}", remote_name, group_name));
    create_group_commit(
        &repo, 
        new_group_tree, 
        &group_reference, 
