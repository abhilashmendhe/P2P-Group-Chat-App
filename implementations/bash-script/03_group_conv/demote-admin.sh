...
if [ -z $REMOTE ]; then 
    echo "Please specify remote name to demote the admin of the group!"
    exit 1
fi
...
if [[ "$LOCAL" == "$REMOTE" ]];then 
    echo "You are self demoting!"
    SELFDEMOTE=1
    # exit 1
fi

if [ $SELFDEMOTE -eq 0 ];then
...
if [[ $SELFDEMOTE -eq 1 ]]; then
    
    echo "Self Demote - $LOCAL"
    if [[ $((local_member_val % 2)) -eq 0 ]]; then 
        echo "$LOCAL - You are not inside the group, can't self-demote!"
        exit 1
    fi
    ...
    echo "Member to demote from admin - $REMOTE"
    if [[ $((admin_causal_val % 2)) -eq 0 ]]; then 
        echo "You are not the admin of the group!!!"
        echo "Can't demote other members as admin!!!"
        exit 1
    fi
    ...
    if [[ -z $IS_MEMBER_PRESENT ]]; then 
        ...
        echo "Please provide a valid member that is in the group to demote admin."
        exit 1
    fi
    ...
    if [[ $((remote_member_val % 2)) -eq 0 ]];then
        echo "Remote Member is not in the group. Can't do this operation."
        echo "Please provide a valid member that is in the group to demote from admin."
        exit 1
    fi
    ...
    if [[ $((admin_remote_causal_val % 2)) -eq 0 ]]; then
        echo "$REMOTE member is not an admin. Can't do this operation."
        exit 1
    fi
    ...
    # echo "new value for $REMTOE - $remote_new_full_value"
    hash_remote_new_full_value=`echo -n $remote_new_full_value | git hash-object --stdin -w`
    # echo "new hash for $REMOTE - $hash_remote_new_full_value"
...
if [[ $NEW_group_state_tree != $group_state_tree ]];then
    ...
    # create message
    MESSAGE="$REMOTE is no more admin for this $GROUP_NAME"
    ...
    COMMIT_MSG="GROUP_OPS($MESSAGE,unmake_admin)"
