...
use crate::member_demote::demote_member_func;


/* 
    To exec: cargo watch -q -c -w src/6_demote -x build
*/
...
fn main() {
    ...
    if args.len() < 3 {
        panic!("Please provide the remote peer name to demote to a normal user of the group.");
    }
    ...
    if !if_peer_admin(&repo, &curr_index, &local_path_str) {
        println!("`{}` does not exists OR is not an admin of the group `{}`. Can't perform 'add' operation.",local_peer_name,group_name); 
        exit(1);
    }

    // 6. Check if remote peer already exists in the group OR existed peer is demoted to an normal member
    let remote_path_str = format!("{}/{}", group_id, remote_name);
    let tup_value = demote_member_func(&repo, &mut curr_index, &remote_path_str, group_name, remote_name);
    ...
    // 9. Create a new commit message
    let commit_message = format!("GROUP_OPS({},demote)",format!("{} is demoted to a normal member of the group {}", remote_name, group_name));
    create_group_commit(
        &repo, 
        new_group_tree, 
        &group_reference, 
