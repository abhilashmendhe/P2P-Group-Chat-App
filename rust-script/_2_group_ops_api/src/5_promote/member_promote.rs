...
pub fn promote_member_func(
    ...
    if let Some(ie) = remote_index_entry {
        ...
        if remote_member_value % 2 == 0 {
            println!("{} is currently not present inside the group!. Can't promote!", remote_name);
            exit(1);
        }

        // Now check if member is already an admin or not
        if remote_admin_value % 2 == 1 {
            println!("{} is already an admin of the group '{}'. Can't promote!", remote_name, group_name);
            exit(1);
        } else {
