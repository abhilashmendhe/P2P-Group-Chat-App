...
use crate::member_promote::promote_member_func;

/* 
    To exec: cargo watch -q -c -w src/5_promote -x build
*/
...
fn main() {
    ...
    if args.len() < 3 {
        panic!("Please provide the remote peer name to promote to an admin of the group.");
    }
    ...
    // 6. Check if remote peer already exists in the group OR existed peer is promoted to an admin
    let remote_path_str = format!("{}/{}", group_id, remote_name);
    let tup_value = promote_member_func(&repo, &mut curr_index, &remote_path_str, group_name, remote_name);
    ...
    // 9. Create commit message
    let commit_message = format!("GROUP_OPS({},promote)",format!("{} is promoted to the admin of the group {}", remote_name, group_name));
    create_group_commit(
        &repo, 
        new_group_tree, 
        &group_reference, 
