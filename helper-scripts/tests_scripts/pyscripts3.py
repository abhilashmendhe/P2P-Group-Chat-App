...
def run_simulation_prd(start, end, gname, ops_type, arr, f: _io.TextIOWrapper):
...
if __name__ == "__main__":
    ...
    parr = random.sample(range(0, 50), 15)
    print(f"Promoting {15} random members from the group: {group_name}")
    run_simulation_prd(0, 50, group_name, 0, parr, f)
    print("\n\n")
    
    # 2.3 Add remaining members
    start = 64
    end = start + 28
    arr = random.sample(range(start, end), 28)
    print(f"Re-adding {end-start} random members in group: {group_name}")
    ...
    # # result = change_group_name(group_name, ngroup_name,  ngroup_desc)
    # # result.insert(0, get_curr_time())
    ...
    dem_arr = random.sample(rdarr, 20)
    print(f"Demoting {20} random admins from the group: {group_name}")
    run_simulation_prd(0, group_size, group_name, 1, dem_arr, f)
    rem_arr = random.sample(rdarr, 20)
    print(f"Removing {20} random members from the group: {group_name}")
    run_simulation_prd(0, group_size, group_name, 2, rem_arr, f)
    
    time.sleep(2)
    # # 2.6 Add only one group member
    start = 100
    end = 200
    arr = random.sample(range(100, 200), 100)
    print(f"Re-adding {100} random members in the group: {group_name}")
    run_simulation_add(100, 200, group_name, arr, f)
